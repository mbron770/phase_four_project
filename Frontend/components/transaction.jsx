...
export default function Transaction({ transactions }) {
  ...
  const filteredTransactionCodes = Object.keys(groupedTransactions).filter(transactionCode => {
    return transactionCode.includes(searchTerm) || groupedTransactions[transactionCode].some(transaction =>
      transaction.product.product_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      transaction.product.product_quantity.toString().includes(searchTerm) ||
      transaction.product.price.toString().includes(searchTerm) ||
      new Date(transaction.transaction_date).toLocaleDateString().includes(searchTerm)
